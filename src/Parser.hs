...
jimpleClassName = try jimpleFullIdentifier
...
jimpleExtendClause = try jimpleJustExtendClause
                  <|> return Nothing
...
jimpleThrowsClause = try jimpleJustThrowsClause
                  <|> return Nothing
...
jimpleModifier = try jimpleModifierAbstract
...
jimpleTypeNonVoidBase = try jimpleNonVoidBoolean
...
jimpleJimpleType = try jimpleJimpleTypeUnknown
...
-- Helper ADT
data JimpleNamedSignature = JimpleNamedSignature Name
...
jimpleImmediate = try (Local <$> identifier)
...
jimpleInvokeExpr = try jimpleStatementSpecialInvoke
...
jimpleStatement = try jimpleStatementInvoke
...
jimpleExpression = try (New <$> jimpleNew)
...
jimpleClassMember = try jimpleClassMemberField
